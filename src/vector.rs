...
impl Vector {
    ...
    /// Constructs from components and a name.
    ...
    /// Constructs the unit vector along x.
    pub fn x_axis() -> Self {
        Self::new(1.0, 0.0, 0.0)
    }

    /// Constructs the unit vector along y.
    pub fn y_axis() -> Self {
        Self::new(0.0, 1.0, 0.0)
    }

    /// Constructs the unit vector along z.
    pub fn z_axis() -> Self {
        Self::new(0.0, 0.0, 1.0)
    }

    /// Constructs the vector from p0 to p1.
    ...
    /// Transforms in place; only rotation and scale apply, a vector has no position.
    ...
    /// Returns the magnitude scaled to stay finite for large components.
    ...
    /// Sets unit length in place; false when the magnitude is zero.
    ...
    /// Returns the angle to other, negated when the cross product points down z; zero below tolerance.
    pub fn angle(
        &self,
        other: &Vector,
    ...
    /// Returns the projection onto projection_vector: (projection, projected length, perpendicular, perpendicular length).
    pub fn projection(
        &self,
    ...
    /// Returns 1 when parallel, -1 when antiparallel, 0 otherwise.
    ...
    /// Returns whether the dot product is within tolerance of zero.
    pub fn is_perpendicular_to(&self, other: &Vector) -> bool {
        self.dot(other).abs() < Tolerance::ZERO_TOLERANCE
    }

    /// Sets this vector perpendicular to v; false when v is zero.
    ...
    /// Returns whether the magnitude is within tolerance of zero.
    pub fn is_zero(&self) -> bool {
        self.compute_magnitude() < Tolerance::ZERO_TOLERANCE
    }

    /// Returns a copy scaled along its direction so its rise along z equals vertical_height.
    ...
    /// Returns the angles to the x, y and z axes.
    ...
    /// Returns the polar angle from z and the azimuth from x.
    ...
    /// Returns the angle in degrees of the xy projection from the x-axis.
    ...
    /// Returns the component-wise average; empty input returns zero.
    ...
    /// Scales in place by 1 / SCALE.
    pub fn scale_down(&mut self) {
        self.scale(1.0 / SCALE);
    }

    /// Returns the reflection through the plane with the given unit normal.
    ...
    /// Returns the unit area-weighted normal of a polygon by Newell's method.
    pub fn average_normal(points: &[Point]) -> Vector {
        ...
        let closed = (&points[last] - &points[0]).magnitude_squared() < 1e-10;
        let n = if closed {
            points.len() - 1
        } else {
            points.len()
    ...
    /// Returns the unit area-weighted normal of a closed polyline by Newell's method.
    ...
    /// Returns the third side from two sides and the angle between them.
    pub fn cosine_law(
    ...
    /// Returns the angle opposite side b from side a, the angle opposite a and side b.
    ...
    /// Returns side b from side a and the angles opposite a and b.
    ...
    /// Returns the angle opposite side c from the three sides.
    ...
    /// Returns the side opposite the first angle from two angles and the side opposite the second.
    ...
    /// Returns "Vector(name, x, y, z, magnitude)".
    ...
    /// Returns GPU-ready [x, y, z] as f32, the f64 to f32 boundary for wgpu upload.
