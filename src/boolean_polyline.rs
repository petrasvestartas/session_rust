...
struct Sc {
    ...
    opt: Vec<VOutPt>,
    orc: Vec<VOutRec>,
    locmin: Vec<VLocalMinima>,
    isnodes: Vec<VIntersectNode>,
    hsegs: Vec<VHorzSeg>,
    hjoins: Vec<VHorzJoin>,
    orclist: Vec<usize>,
    scan: Vec<i64>,
    actives: OptIdx,
    sel: OptIdx,
...
impl Sc {
    ...
    fn alloc_vtx(&mut self) -> usize {
        self.vtx.push(VVertex::default());
        self.vtx.len() - 1
    }
    fn alloc_act(&mut self) -> usize {
        self.act.push(VActive::default());
        self.act.len() - 1
    }
    fn alloc_opt(&mut self, pt: Pt, outrec: usize) -> usize {
        let i = self.opt.len();
        self.opt.push(VOutPt {
            pt,
            next: i,
            prev: i,
            outrec,
    ...
    fn pop_scan(&mut self) -> Option<i64> {
        ...
        loop {
            let (l, r) = (2 * i + 1, 2 * i + 2);
            let mut m = i;
            if l < n && self.scan[l] > self.scan[m] {
                m = l;
            }
            if r < n && self.scan[r] > self.scan[m] {
                m = r;
            }
            if m == i {
                break;
    ...
    fn pop_lm(&mut self, y: i64) -> Option<VLocalMinima> {
        if self.lm_idx >= self.locmin.len() {
            return None;
        }
        let vtx_idx = self.locmin[self.lm_idx].vertex;
        if self.vtx[vtx_idx].pt.y != y {
            return None;
        }
        let lm = self.locmin[self.lm_idx].clone();
        self.lm_idx += 1;
        Some(lm)
    }
}
...
fn is_horz(sc: &Sc, e: usize) -> bool {
    sc.act[e].top.y == sc.act[e].bot.y
}
fn is_hot(sc: &Sc, e: usize) -> bool {
    sc.act[e].outrec.is_some()
}
fn is_maxima_v(sc: &Sc, v: usize) -> bool {
    sc.vtx[v].flags & VF_LOCAL_MAX != 0
}
fn is_maxima(sc: &Sc, e: usize) -> bool {
    sc.act[e].vertex_top.map_or(false, |v| is_maxima_v(sc, v))
}
fn is_front(sc: &Sc, e: usize) -> bool {
    sc.act[e]
        .outrec
        .map_or(false, |o| sc.orc[o].front_edge == Some(e))
}
fn is_joined(sc: &Sc, e: usize) -> bool {
    sc.act[e].join_with != 0
}
fn polytype(sc: &Sc, e: usize) -> i8 {
    sc.act[e].local_min.map_or(0, |lm| sc.locmin[lm].polytype)
}
fn same_pt(sc: &Sc, e1: usize, e2: usize) -> bool {
    polytype(sc, e1) == polytype(sc, e2)
}
...
fn prev_prev_vertex(sc: &Sc, e: usize) -> OptIdx {
    ...
    if sc.act[e].wind_dx > 0 {
        sc.vtx[sc.vtx[vt].prev?].prev
    } else {
        sc.vtx[sc.vtx[vt].next?].next
    }
}

fn cross3(p1: Pt, p2: Pt, p3: Pt) -> f64 {
    (p2.x - p1.x) as f64 * (p3.y - p2.y) as f64 - (p2.y - p1.y) as f64 * (p3.x - p2.x) as f64
}
fn dot3(p1: Pt, p2: Pt, p3: Pt) -> f64 {
...
fn perpendic_dist_sq(pt: Pt, l1: Pt, l2: Pt) -> f64 {
    let (a, b, c, d) = (
        (pt.x - l1.x) as f64,
        (pt.y - l1.y) as f64,
        (l2.x - l1.x) as f64,
...
fn closest_on_seg(pt: Pt, s1: Pt, s2: Pt) -> Pt {
    ...
    let mut q = ((pt.x - s1.x) as f64 * dx + (pt.y - s1.y) as f64 * dy) / (dx * dx + dy * dy);
    if q < 0.0 {
        q = 0.0;
    } else if q > 1.0 {
        q = 1.0;
...
fn segs_intersect(a: Pt, b: Pt, c: Pt, d: Pt) -> bool {
    ...
    (sign(cross3(a, c, d)) * sign(cross3(b, c, d)) < 0)
        && (sign(cross3(c, a, b)) * sign(cross3(d, a, b)) < 0)
}

fn area_outpt(sc: &Sc, start: usize) -> f64 {
    let mut r = 0.0;
    let mut o = start;
    loop {
        let prev = sc.opt[o].prev;
        let pp = sc.opt[prev].pt;
        let cp = sc.opt[o].pt;
...
fn area_tri(p1: Pt, p2: Pt, p3: Pt) -> f64 {
    (p3.y + p1.y) as f64 * (p3.x - p1.x) as f64
        + (p1.y + p2.y) as f64 * (p1.x - p2.x) as f64
        + (p2.y + p3.y) as f64 * (p2.x - p3.x) as f64
}

fn pts_close(a: Pt, b: Pt) -> bool {
    (a.x - b.x).abs() < 2 && (a.y - b.y).abs() < 2
}

fn very_small_tri(sc: &Sc, op: usize) -> bool {
    let n = sc.opt[op].next;
    let nn = sc.opt[n].next;
    let p = sc.opt[op].prev;
    nn == p
        && (pts_close(sc.opt[p].pt, sc.opt[n].pt)
            || pts_close(sc.opt[op].pt, sc.opt[n].pt)
            || pts_close(sc.opt[op].pt, sc.opt[p].pt))
}

fn valid_closed(sc: &Sc, op: usize) -> bool {
    let n = sc.opt[op].next;
    n != op && n != sc.opt[op].prev && !very_small_tri(sc, op)
}
...
fn pip_vertex(sc: &Sc, pt: Pt, head: usize) -> bool {
    ...
    loop {
        let a = sc.vtx[v].pt;
        let b = sc.vtx[sc.vtx[v].next.unwrap()].pt;
        let cross =
            (b.x - a.x) as i128 * (pt.y - a.y) as i128 - (b.y - a.y) as i128 * (pt.x - a.x) as i128;
        if a.y <= pt.y {
            if b.y > pt.y && cross > 0 {
...
fn add_path_from_coords(
    ...
    let cvt = |i: usize| -> Pt {
        Pt {
            x: (ca[i * 3] * bool_scale).round() as i64,
            y: (ca[i * 3 + 1] * bool_scale).round() as i64,
        }
    };
    let pt0 = cvt(0);
    sc.vtx[base].pt = pt0;
    ...
    for i in 1..n {
        let pt = cvt(i);
        if pt == sc.vtx[prev].pt {
            continue;
        }
        let ci = base + cnt;
        sc.vtx[ci].pt = pt;
        sc.vtx[ci].prev = Some(prev);
        sc.vtx[prev].next = Some(ci);
        prev = ci;
        cnt += 1;
        if pt.x < min_x {
            min_x = pt.x;
    ...
    if cnt < 3 {
        sc.vtx.truncate(base);
        return (None, 0, 0, 0, 0);
    }
    if sc.vtx[prev].pt == sc.vtx[base].pt {
        prev = sc.vtx[prev].prev.unwrap();
        cnt -= 1;
    }
    if cnt < 3 {
        sc.vtx.truncate(base);
    ...
    sc.vtx[prev].next = Some(base);
    sc.vtx[base].prev = Some(prev);
    // local minima detection
    let mut pv = sc.vtx[base].prev.unwrap();
    while pv != base && sc.vtx[pv].pt.y == sc.vtx[base].pt.y {
        pv = sc.vtx[pv].prev.unwrap();
    }
    if pv == base {
        return (Some(base), min_x, max_x, min_y, max_y);
    }
    ...
    while cv != base {
        if sc.vtx[cv].pt.y > sc.vtx[pv].pt.y && going_up {
            sc.vtx[pv].flags |= VF_LOCAL_MAX;
            going_up = false;
        } else if sc.vtx[cv].pt.y < sc.vtx[pv].pt.y && !going_up {
            going_up = true;
            sc.vtx[pv].flags |= VF_LOCAL_MIN;
            sc.locmin.push(VLocalMinima {
                vertex: pv,
                polytype: pty,
            });
        }
        pv = cv;
        cv = sc.vtx[cv].next.unwrap();
    }
...
fn get_maxima_pair(sc: &Sc, e: usize) -> OptIdx {
    ...
    while let Some(ei) = e2 {
        if sc.act[ei].vertex_top == Some(vt) {
            return Some(ei);
        }
...
fn get_curr_y_maxima(sc: &Sc, e: usize) -> OptIdx {
    ...
    if sc.act[e].wind_dx > 0 {
        while sc.vtx[sc.vtx[r].next?].pt.y == sc.vtx[r].pt.y {
            r = sc.vtx[r].next?;
        }
    } else {
        while sc.vtx[sc.vtx[r].prev?].pt.y == sc.vtx[r].pt.y {
            r = sc.vtx[r].prev?;
        }
    }
    if is_maxima_v(sc, r) {
...
fn get_prev_hot(sc: &Sc, e: usize) -> OptIdx {
    ...
    while let Some(pi) = p {
        ...
        p = sc.act[pi].prev_in_ael;
    }
    None
}

fn is_valid_ael_order(sc: &Sc, res: usize, new: usize) -> bool {
    if sc.act[new].curr_x != sc.act[res].curr_x {
        return sc.act[new].curr_x > sc.act[res].curr_x;
    }
    ...
    if !is_maxima(sc, res) && sc.act[res].top.y > sc.act[new].top.y {
        return cross3(
            sc.act[new].bot,
            sc.act[res].top,
            sc.vtx[next_vertex(sc, res).unwrap()].pt,
        ) <= 0.0;
    }
    if !is_maxima(sc, new) && sc.act[new].top.y > sc.act[res].top.y {
        return cross3(
            sc.act[new].bot,
            sc.act[new].top,
            sc.vtx[next_vertex(sc, new).unwrap()].pt,
        ) >= 0.0;
    }
    let y = sc.act[new].bot.y;
    if sc.act[res].bot.y != y
        || sc.vtx[sc.locmin[sc.act[res].local_min.unwrap()].vertex]
            .pt
            .y
            != y
    {
        return sc.act[new].is_left_bound;
    }
    if sc.act[res].is_left_bound != sc.act[new].is_left_bound {
        return sc.act[new].is_left_bound;
    }
    let pp_res = prev_prev_vertex(sc, res);
    if pp_res.is_some()
        && is_collinear(sc.vtx[pp_res.unwrap()].pt, sc.act[res].bot, sc.act[res].top)
    {
        return true;
    }
    ...
    if pp_res.is_some() && pp_new.is_some() {
        (cross3(
            sc.vtx[pp_res.unwrap()].pt,
            sc.act[new].bot,
            sc.vtx[pp_new.unwrap()].pt,
        ) > 0.0)
            == sc.act[new].is_left_bound
    } else {
        sc.act[new].is_left_bound
    }
}
...
fn insert_left_edge(sc: &mut Sc, e: usize) {
    if sc.actives.is_none() {
        sc.act[e].prev_in_ael = None;
        sc.act[e].next_in_ael = None;
        sc.actives = Some(e);
    } else if !is_valid_ael_order(sc, sc.actives.unwrap(), e) {
        sc.act[e].prev_in_ael = None;
        sc.act[e].next_in_ael = sc.actives;
        sc.act[sc.actives.unwrap()].prev_in_ael = Some(e);
        sc.actives = Some(e);
    } else {
        let mut e2 = sc.actives.unwrap();
        while sc.act[e2].next_in_ael.is_some()
            && is_valid_ael_order(sc, sc.act[e2].next_in_ael.unwrap(), e)
        {
            e2 = sc.act[e2].next_in_ael.unwrap();
        }
        if sc.act[e2].join_with == 2 {
            e2 = sc.act[e2].next_in_ael.unwrap();
        }
        let next = sc.act[e2].next_in_ael;
        sc.act[e].next_in_ael = next;
        if let Some(n) = next {
            sc.act[n].prev_in_ael = Some(e);
        }
        sc.act[e].prev_in_ael = Some(e2);
        sc.act[e2].next_in_ael = Some(e);
    }
}

fn insert_right_edge(sc: &mut Sc, e: usize, e2: usize) {
    let next = sc.act[e].next_in_ael;
    sc.act[e2].next_in_ael = next;
    if let Some(n) = next {
        sc.act[n].prev_in_ael = Some(e2);
    }
    sc.act[e2].prev_in_ael = Some(e);
    sc.act[e].next_in_ael = Some(e2);
}

fn swap_positions_in_ael(sc: &mut Sc, e1: usize, e2: usize) {
    let next = sc.act[e2].next_in_ael;
    if let Some(n) = next {
        sc.act[n].prev_in_ael = Some(e1);
    }
    let prev = sc.act[e1].prev_in_ael;
    if let Some(p) = prev {
        sc.act[p].next_in_ael = Some(e2);
    }
    sc.act[e2].prev_in_ael = prev;
    sc.act[e2].next_in_ael = Some(e1);
    sc.act[e1].prev_in_ael = Some(e2);
    sc.act[e1].next_in_ael = next;
    if sc.act[e2].prev_in_ael.is_none() {
        sc.actives = Some(e2);
    }
}

fn delete_from_ael(sc: &mut Sc, e: usize) {
    let prev = sc.act[e].prev_in_ael;
    let next = sc.act[e].next_in_ael;
    if prev.is_none() && next.is_none() && sc.actives != Some(e) {
        return;
    }
    if let Some(p) = prev {
        sc.act[p].next_in_ael = next;
    } else {
        sc.actives = next;
    }
    if let Some(n) = next {
        sc.act[n].prev_in_ael = prev;
    }
}
...
fn set_wind_count(sc: &mut Sc, e: usize) {
    ...
    while e2.is_some() && polytype(sc, e2.unwrap()) != pt {
        e2 = sc.act[e2.unwrap()].prev_in_ael;
    }
    if e2.is_none() {
        ...
        let ei = e2.unwrap();
        let wc = sc.act[ei].wind_cnt;
        let wd = sc.act[ei].wind_dx;
        let ewd = sc.act[e].wind_dx;
        if wc * wd < 0 {
            sc.act[e].wind_cnt = if wc.abs() > 1 {
                if wd * ewd < 0 {
                    wc
                } else {
                    wc + ewd
    ...
    while e2.is_some() && e2.unwrap() != e {
        let ei = e2.unwrap();
        if polytype(sc, ei) != pt {
            sc.act[e].wind_cnt2 += sc.act[ei].wind_dx;
        }
        e2 = sc.act[ei].next_in_ael;
    }
}
...
fn set_sides(sc: &mut Sc, or: usize, f: usize, b: usize) {
    sc.orc[or].front_edge = Some(f);
    sc.orc[or].back_edge = Some(b);
}
...
fn swap_outrecs(sc: &mut Sc, e1: usize, e2: usize) {
    ...
    if or1 == or2 {
        if let Some(o) = or1 {
            let fe = sc.orc[o].front_edge;
            sc.orc[o].front_edge = sc.orc[o].back_edge;
            sc.orc[o].back_edge = fe;
        }
        return;
    }
    if let Some(o) = or1 {
        if sc.orc[o].front_edge == Some(e1) {
            sc.orc[o].front_edge = Some(e2);
        } else {
            sc.orc[o].back_edge = Some(e2);
        }
    }
    if let Some(o) = or2 {
        if sc.orc[o].front_edge == Some(e2) {
            sc.orc[o].front_edge = Some(e1);
        } else {
            sc.orc[o].back_edge = Some(e1);
        }
    }
    sc.act[e1].outrec = or2;
    sc.act[e2].outrec = or1;
}
...
fn add_outpt(sc: &mut Sc, e: usize, pt: Pt) -> usize {
    ...
    if to_front && pt == sc.opt[op_front].pt {
        return op_front;
    }
    if !to_front && pt == sc.opt[op_back].pt {
        return op_back;
    }
    let nop = sc.alloc_opt(pt, or);
    sc.opt[op_back].prev = nop;
    sc.opt[nop].prev = op_front;
    sc.opt[nop].next = op_back;
    sc.opt[op_front].next = nop;
    if to_front {
        sc.orc[or].pts = Some(nop);
    }
    nop
}
...
fn add_local_min_poly(sc: &mut Sc, e1: usize, e2: usize, pt: Pt, is_new: bool) -> usize {
    let or = sc.alloc_orc();
    sc.act[e1].outrec = Some(or);
    sc.act[e2].outrec = Some(or);
    let prev_hot = get_prev_hot(sc, e1);
    if let Some(ph) = prev_hot {
        let ascending = sc.act[ph]
            .outrec
            .map_or(false, |o| sc.orc[o].front_edge == Some(ph));
        if ascending == is_new {
            set_sides(sc, or, e2, e1);
        } else {
            set_sides(sc, or, e1, e2);
        }
    } else {
...
fn uncouple(sc: &mut Sc, e: usize) {
    if let Some(or) = sc.act[e].outrec {
        if let Some(fe) = sc.orc[or].front_edge {
            sc.act[fe].outrec = None;
        }
        if let Some(be) = sc.orc[or].back_edge {
            sc.act[be].outrec = None;
        }
        sc.orc[or].front_edge = None;
        sc.orc[or].back_edge = None;
    }
}

fn join_outrec_paths(sc: &mut Sc, e1: usize, e2: usize) {
    let or1 = sc.act[e1].outrec.unwrap();
    let or2 = sc.act[e2].outrec.unwrap();
    let p1_st = sc.orc[or1].pts.unwrap();
    let p2_st = sc.orc[or2].pts.unwrap();
    let p1_end = sc.opt[p1_st].next;
    let p2_end = sc.opt[p2_st].next;
    if is_front(sc, e1) {
        sc.opt[p2_end].prev = p1_st;
        sc.opt[p1_st].next = p2_end;
        sc.opt[p2_st].next = p1_end;
        sc.opt[p1_end].prev = p2_st;
        sc.orc[or1].pts = Some(p2_st);
        sc.orc[or1].front_edge = sc.orc[or2].front_edge;
        if let Some(fe) = sc.orc[or1].front_edge {
            sc.act[fe].outrec = Some(or1);
        }
    } else {
        sc.opt[p1_end].prev = p2_st;
        sc.opt[p2_st].next = p1_end;
        ...
        if let Some(be) = sc.orc[or1].back_edge {
            sc.act[be].outrec = Some(or1);
        }
    }
    sc.orc[or2].front_edge = None;
    sc.orc[or2].back_edge = None;
    sc.orc[or2].pts = None;
    sc.orc[or2].owner = Some(or1);
    sc.act[e1].outrec = None;
    sc.act[e2].outrec = None;
}
...
fn add_local_max_poly(sc: &mut Sc, e1: usize, e2: usize, pt: Pt) -> OptIdx {
    ...
    if is_front(sc, e1) == is_front(sc, e2) {
        sc.ok = false;
        return None;
    }
    ...
    if sc.act[e1].outrec == sc.act[e2].outrec {
        ...
        uncouple(sc, e1);
    } else if sc.act[e1].outrec.map_or(0, |o| sc.orc[o].idx)
        < sc.act[e2].outrec.map_or(0, |o| sc.orc[o].idx)
    {
        join_outrec_paths(sc, e1, e2);
    } else {
        join_outrec_paths(sc, e2, e1);
    }
    Some(result)
}
...
fn check_join_left(sc: &mut Sc, e: usize, pt: Pt, check_cx: bool) {
    let prev = match sc.act[e].prev_in_ael {
        Some(p) => p,
        None => return,
    };
    if !is_hot(sc, e) || !is_hot(sc, prev) || is_horz(sc, e) || is_horz(sc, prev) {
        return;
    }
    if (pt.y < sc.act[e].top.y + 2 || pt.y < sc.act[prev].top.y + 2)
        && (sc.act[e].bot.y > pt.y || sc.act[prev].bot.y > pt.y)
    {
        return;
    }
    if check_cx {
        if perpendic_dist_sq(pt, sc.act[prev].bot, sc.act[prev].top) > 0.25 {
            return;
        }
    } else if sc.act[e].curr_x != sc.act[prev].curr_x {
        return;
    }
    if !is_collinear(sc.act[e].top, pt, sc.act[prev].top) {
        return;
    }
    let or_e = sc.act[e]
        .outrec
        .map(|o| sc.orc[o].idx)
        .unwrap_or(usize::MAX);
    let or_p = sc.act[prev]
        .outrec
        .map(|o| sc.orc[o].idx)
        .unwrap_or(usize::MAX);
    if or_e == or_p {
        add_local_max_poly(sc, prev, e, pt);
    } else if or_e < or_p {
        join_outrec_paths(sc, e, prev);
    } else {
        join_outrec_paths(sc, prev, e);
    }
    sc.act[prev].join_with = 2;
    sc.act[e].join_with = 1;
}

fn check_join_right(sc: &mut Sc, e: usize, pt: Pt, check_cx: bool) {
    let next = match sc.act[e].next_in_ael {
        Some(n) => n,
        None => return,
    };
    if !is_hot(sc, e) || !is_hot(sc, next) || is_horz(sc, e) || is_horz(sc, next) {
        return;
    }
    if (pt.y < sc.act[e].top.y + 2 || pt.y < sc.act[next].top.y + 2)
        && (sc.act[e].bot.y > pt.y || sc.act[next].bot.y > pt.y)
    {
        return;
    }
    if check_cx {
        if perpendic_dist_sq(pt, sc.act[next].bot, sc.act[next].top) > 0.35 {
            return;
        }
    } else if sc.act[e].curr_x != sc.act[next].curr_x {
        return;
    }
    if !is_collinear(sc.act[e].top, pt, sc.act[next].top) {
        return;
    }
    let or_e = sc.act[e]
        .outrec
        .map(|o| sc.orc[o].idx)
        .unwrap_or(usize::MAX);
    let or_n = sc.act[next]
        .outrec
        .map(|o| sc.orc[o].idx)
        .unwrap_or(usize::MAX);
    if or_e == or_n {
        add_local_max_poly(sc, e, next, pt);
    } else if or_e < or_n {
        join_outrec_paths(sc, e, next);
    } else {
        join_outrec_paths(sc, next, e);
    }
    sc.act[e].join_with = 2;
    sc.act[next].join_with = 1;
}
...
fn intersect_edges(sc: &mut Sc, e1: usize, e2: usize, pt: Pt, ct: i32) {
    ...
    let ow1 = sc.act[e1].wind_cnt.abs();
    let ow2 = sc.act[e2].wind_cnt.abs();
    let in01_1 = ow1 == 0 || ow1 == 1;
    let in01_2 = ow2 == 0 || ow2 == 1;
    if (!is_hot(sc, e1) && !in01_1) || (!is_hot(sc, e2) && !in01_2) {
        return;
    }
    ...
    } else if is_hot(sc, e1) {
        add_outpt(sc, e1, pt);
        swap_outrecs(sc, e1, e2);
    } else if is_hot(sc, e2) {
        add_outpt(sc, e2, pt);
        swap_outrecs(sc, e1, e2);
    } else {
        let wc2_1 = sc.act[e1].wind_cnt2.abs() as i64;
        let wc2_2 = sc.act[e2].wind_cnt2.abs() as i64;
        if !same_pt(sc, e1, e2) {
            add_local_min_poly(sc, e1, e2, pt, false);
        } else if ow1 == 1 && ow2 == 1 {
            match ct {
                ...
                _ => {
                    if (polytype(sc, e1) == 1 && wc2_1 > 0 && wc2_2 > 0)
                        || (polytype(sc, e1) == 0 && wc2_1 <= 0 && wc2_2 <= 0)
                    {
                        add_local_min_poly(sc, e1, e2, pt, false);
                    }
                }
...
fn update_edge_into_ael(sc: &mut Sc, e: usize, _ct: i32) {
    ...
    sc.act[e].bot = sc.act[e].top;
    sc.act[e].vertex_top = Some(nv);
    sc.act[e].top = sc.vtx[nv].pt;
    sc.act[e].curr_x = sc.act[e].bot.x;
    sc.act[e].dx = get_dx(sc.act[e].bot, sc.act[e].top);
    if is_joined(sc, e) {
        split(sc, e, sc.act[e].bot);
    }
    if is_horz(sc, e) {
        // trim horz
        let mut pt = sc.vtx[next_vertex(sc, e).unwrap()].pt;
        while pt.y == sc.act[e].top.y {
            if (pt.x < sc.act[e].top.x) != (sc.act[e].bot.x < sc.act[e].top.x) {
                break;
            }
...
fn reset_horz_dir(sc: &Sc, e: usize, max_v: OptIdx) -> (bool, i64, i64) {
    if sc.act[e].bot.x == sc.act[e].top.x {
        ...
        while ae.is_some() && sc.act[ae.unwrap()].vertex_top != max_v {
            ae = sc.act[ae.unwrap()].next_in_ael;
        }
        (ae.is_some(), sc.act[e].curr_x, sc.act[e].curr_x)
    } else if sc.act[e].curr_x < sc.act[e].top.x {
        (true, sc.act[e].curr_x, sc.act[e].top.x)
    } else {
        (false, sc.act[e].top.x, sc.act[e].curr_x)
    }
}
...
fn do_horizontal(sc: &mut Sc, horz: usize, ct: i32) {
    ...
    if is_hot(sc, horz) {
        let op = add_outpt(
            ...
            horz,
            Pt {
                x: sc.act[horz].curr_x,
                y,
            },
        );
        sc.hsegs.push(VHorzSeg {
            left_op: op,
            right_op: None,
            left_to_right: true,
        });
    ...
    loop {
        let mut ei = if is_ltr {
            sc.act[horz].next_in_ael
        } else {
            sc.act[horz].prev_in_ael
        };
        while let Some(e) = ei {
            if sc.act[e].vertex_top == vertex_max {
                if is_hot(sc, horz) && is_joined(sc, e) {
                    split(sc, e, sc.act[e].top);
                }
                if is_hot(sc, horz) {
                    ...
                    if is_ltr {
                        add_local_max_poly(sc, horz, e, sc.act[horz].top);
                    } else {
                        add_local_max_poly(sc, e, horz, sc.act[horz].top);
                    }
                }
                delete_from_ael(sc, e);
                delete_from_ael(sc, horz);
                return;
            }
            if vertex_max != sc.act[horz].vertex_top {
                if (is_ltr && sc.act[e].curr_x > hr) || (!is_ltr && sc.act[e].curr_x < hl) {
                    break;
                }
                if sc.act[e].curr_x == sc.act[horz].top.x && !is_horz(sc, e) {
                    let pt2 = sc.vtx[next_vertex(sc, horz).unwrap()].pt;
                    if is_ltr {
                        if top_x(sc, e, pt2.y) >= pt2.x {
                            break;
                        }
                    } else {
                        if top_x(sc, e, pt2.y) <= pt2.x {
            ...
            if is_ltr {
                intersect_edges(sc, horz, e, pt, ct);
                swap_positions_in_ael(sc, horz, e);
                check_join_left(sc, e, pt, false);
                sc.act[horz].curr_x = sc.act[e].curr_x;
                ei = sc.act[horz].next_in_ael;
            } else {
                intersect_edges(sc, e, horz, pt, ct);
                swap_positions_in_ael(sc, e, horz);
                check_join_right(sc, e, pt, false);
                sc.act[horz].curr_x = sc.act[e].curr_x;
                ei = sc.act[horz].prev_in_ael;
            }
            if sc.act[horz].outrec.is_some() {
                let last_op = {
                    let or = sc.act[horz].outrec.unwrap();
                    let pts = sc.orc[or].pts.unwrap();
                    if sc.orc[or].front_edge == Some(horz) {
                        pts
                    } else {
                        sc.opt[pts].next
                    }
        ...
        if nv.is_none() || sc.vtx[nv.unwrap()].pt.y != sc.act[horz].top.y {
            break;
        }
        if is_hot(sc, horz) {
            add_outpt(sc, horz, sc.act[horz].top);
...
fn dup_outpt(sc: &mut Sc, op: usize, after: bool) -> usize {
    ...
    if after {
        let next = sc.opt[op].next;
        sc.opt[r].next = next;
        sc.opt[next].prev = r;
        sc.opt[r].prev = op;
        sc.opt[op].next = r;
    } else {
        let prev = sc.opt[op].prev;
        sc.opt[r].prev = prev;
        sc.opt[prev].next = r;
        sc.opt[r].next = op;
        sc.opt[op].prev = r;
...
fn convert_horz_segs_to_joins(sc: &mut Sc) {
    ...
    for i in 0..n {
        ...
        while sc.orc[or].pts.is_none() {
            or = sc.orc[or].owner.unwrap_or(or);
            if sc.orc[or].pts.is_some() || sc.orc[or].owner.is_none() {
                break;
            }
        }
        if sc.orc[or].pts.is_none() {
            sc.hsegs[i].right_op = None;
            continue;
        }
        ...
        let mut op_n = op;
        if has_edges {
            let op_a = sc.orc[or].pts.unwrap();
            let op_z = sc.opt[op_a].next;
            while op_p != op_z && sc.opt[sc.opt[op_p].prev].pt.y == cy {
                op_p = sc.opt[op_p].prev;
            }
            while op_n != op_a && sc.opt[sc.opt[op_n].next].pt.y == cy {
                op_n = sc.opt[op_n].next;
            }
        } else {
            while sc.opt[op_p].prev != op_n && sc.opt[sc.opt[op_p].prev].pt.y == cy {
                op_p = sc.opt[op_p].prev;
            }
            while sc.opt[op_n].next != op_p && sc.opt[sc.opt[op_n].next].pt.y == cy {
                op_n = sc.opt[op_n].next;
            }
        }
        ...
        if sc.opt[op_p].pt.x < sc.opt[op_n].pt.x {
            sc.hsegs[i].left_op = op_p;
            sc.hsegs[i].right_op = Some(op_n);
            sc.hsegs[i].left_to_right = true;
        } else {
            sc.hsegs[i].left_op = op_n;
            sc.hsegs[i].right_op = Some(op_p);
            sc.hsegs[i].left_to_right = false;
        }
        if sc.opt[sc.hsegs[i].left_op].horz_used {
            sc.hsegs[i].right_op = None;
            continue;
        }
    ...
    sc.hsegs.sort_by(|a, b| {
        ...
        sc.opt[b.left_op]
            .pt
            .x
            .cmp(&sc.opt[a.left_op].pt.x)
            .reverse()
    });
    let j = valid;
    for i in 0..j.saturating_sub(1) {
        for k in (i + 1)..j {
            let hs1_lo = sc.hsegs[i].left_op;
            let hs1_ro = sc.hsegs[i].right_op.unwrap();
            let hs2_lo = sc.hsegs[k].left_op;
            let hs2_ro = sc.hsegs[k].right_op.unwrap();
            if sc.opt[hs2_lo].pt.x >= sc.opt[hs1_ro].pt.x
                || sc.hsegs[k].left_to_right == sc.hsegs[i].left_to_right
                || sc.opt[hs2_ro].pt.x <= sc.opt[hs1_lo].pt.x
            {
                continue;
            }
            let cy = sc.opt[hs1_lo].pt.y;
            let mut lo1 = sc.hsegs[i].left_op;
            let mut lo2 = sc.hsegs[k].left_op;
            if sc.hsegs[i].left_to_right {
                while sc.opt[sc.opt[lo1].next].pt.y == cy
                    && sc.opt[sc.opt[lo1].next].pt.x <= sc.opt[lo2].pt.x
                {
                    lo1 = sc.opt[lo1].next;
                }
                while sc.opt[sc.opt[lo2].prev].pt.y == cy
                    && sc.opt[sc.opt[lo2].prev].pt.x <= sc.opt[lo1].pt.x
                {
                    lo2 = sc.opt[lo2].prev;
                }
                let d1 = dup_outpt(sc, lo1, true);
                let d2 = dup_outpt(sc, lo2, false);
                sc.hjoins.push(VHorzJoin { op1: d1, op2: d2 });
            } else {
                while sc.opt[sc.opt[lo1].prev].pt.y == cy
                    && sc.opt[sc.opt[lo1].prev].pt.x <= sc.opt[lo2].pt.x
                {
                    lo1 = sc.opt[lo1].prev;
                }
                while sc.opt[sc.opt[lo2].next].pt.y == cy
                    && sc.opt[sc.opt[lo2].next].pt.x <= sc.opt[lo1].pt.x
                {
                    lo2 = sc.opt[lo2].next;
                }
                let d1 = dup_outpt(sc, lo2, true);
                let d2 = dup_outpt(sc, lo1, false);
                sc.hjoins.push(VHorzJoin { op1: d1, op2: d2 });
            }
            sc.hsegs[i].left_op = lo1;
            sc.hsegs[k].left_op = lo2;
        }
    }
}

fn fix_outrec_pts(sc: &mut Sc, or: usize) {
    let start = sc.orc[or].pts.unwrap();
    let mut o = start;
    loop {
        sc.opt[o].outrec = or;
        o = sc.opt[o].next;
...
fn process_horz_joins(sc: &mut Sc) {
    for ji in 0..sc.hjoins.len() {
        let op1 = sc.hjoins[ji].op1;
        let op2 = sc.hjoins[ji].op2;
        let mut or1 = sc.opt[op1].outrec;
        while sc.orc[or1].pts.is_none() {
            or1 = sc.orc[or1].owner.unwrap_or(or1);
        }
        let mut or2 = sc.opt[op2].outrec;
        while sc.orc[or2].pts.is_none() {
            or2 = sc.orc[or2].owner.unwrap_or(or2);
        }
        let op1b = sc.opt[op1].next;
        let op2b = sc.opt[op2].prev;
        sc.opt[op1].next = op2;
        sc.opt[op2].prev = op1;
        sc.opt[op1b].prev = op2b;
        sc.opt[op2b].next = op1b;
        if or1 == or2 {
            let nr = sc.alloc_orc();
            sc.orc[nr].pts = Some(op1b);
            fix_outrec_pts(sc, nr);
            if sc.opt[sc.orc[or1].pts.unwrap()].outrec == nr {
                sc.orc[or1].pts = Some(op1);
                sc.opt[op1].outrec = or1;
            }
...
fn add_new_isect_node(sc: &mut Sc, e1: usize, e2: usize, top_y: i64) {
    ...
    if ip.y > sc.bot_y || ip.y < top_y {
        ...
        if ad1 > 100.0 && ad2 > 100.0 {
            ip = if ad1 > ad2 {
                closest_on_seg(ip, sc.act[e1].bot, sc.act[e1].top)
            } else {
                closest_on_seg(ip, sc.act[e2].bot, sc.act[e2].top)
            };
        } else if ad1 > 100.0 {
            ip = closest_on_seg(ip, sc.act[e1].bot, sc.act[e1].top);
        } else if ad2 > 100.0 {
            ip = closest_on_seg(ip, sc.act[e2].bot, sc.act[e2].top);
        } else {
            if ip.y < top_y {
                ip.y = top_y;
            } else {
    ...
    sc.isnodes.push(VIntersectNode {
        pt: ip,
        edge1: e1,
        edge2: e2,
    });
...
fn build_intersect_list(sc: &mut Sc, top_y: i64) -> bool {
    ...
    if sc.act[first].next_in_ael.is_none() {
        return false;
    }
    ...
    while let Some(e) = e_opt {
        ...
        if sc.act[e].join_with == 1 {
            sc.act[e].curr_x = sc.act[sc.act[e].prev_in_ael.unwrap()].curr_x;
        } else {
            sc.act[e].curr_x = top_x(sc, e, top_y);
        }
            ...
            while left != l_end && right != r_end {
                if sc.act[right.unwrap()].curr_x < sc.act[left.unwrap()].curr_x {
                    ...
                    if let Some(rn) = ri_next {
                        sc.act[rn].prev_in_sel = sc.act[ri].prev_in_sel;
                    }
                    ...
                    if let Some(p) = sc.act[ri].prev_in_sel {
                        sc.act[p].next_in_sel = Some(ri);
                    }
                    sc.act[ri].next_in_sel = Some(li);
                    sc.act[li].prev_in_sel = Some(ri);
                    if left == Some(curr_base) {
                        curr_base = ri;
                        sc.act[curr_base].jump = r_end;
                        if prev_base.is_none() {
                            sc.sel = Some(curr_base);
                        } else {
                            sc.act[prev_base.unwrap()].jump = Some(curr_base);
                        }
                    }
                } else {
                    left = sc.act[left.unwrap()].next_in_sel;
                }
            }
...
fn process_intersect_list(sc: &mut Sc, ct: i32) {
    ...
    for i in 0..n {
        let e1 = sc.isnodes[i].edge1;
        let e2 = sc.isnodes[i].edge2;
        let adj = sc.act[e1].next_in_ael == Some(e2) || sc.act[e1].prev_in_ael == Some(e2);
        if !adj {
            for j in (i + 1)..n {
                let je1 = sc.isnodes[j].edge1;
                let je2 = sc.isnodes[j].edge2;
        ...
        let pt = sc.isnodes[i].pt;
        let e1 = sc.isnodes[i].edge1;
        let e2 = sc.isnodes[i].edge2;
        intersect_edges(sc, e1, e2, pt, ct);
        swap_positions_in_ael(sc, e1, e2);
        sc.act[e1].curr_x = pt.x;
        sc.act[e2].curr_x = pt.x;
        check_join_left(sc, e2, pt, true);
        check_join_right(sc, e1, pt, true);
    }
}
...
fn insert_local_minima_into_ael(sc: &mut Sc, bot_y: i64, ct: i32) {
    while let Some(lm) = sc.pop_lm(bot_y) {
        ...
        let lb = sc.alloc_act();
        sc.act[lb].bot = lm_pt;
        sc.act[lb].curr_x = lm_pt.x;
        sc.act[lb].wind_dx = -1;
        sc.act[lb].vertex_top = sc.vtx[lm.vertex].prev;
        sc.act[lb].top = sc.vtx[sc.act[lb].vertex_top.unwrap()].pt;
        sc.act[lb].local_min = Some(lm_idx);
        sc.act[lb].dx = get_dx(sc.act[lb].bot, sc.act[lb].top);
        let rb = sc.alloc_act();
        sc.act[rb].bot = lm_pt;
        sc.act[rb].curr_x = lm_pt.x;
        sc.act[rb].wind_dx = 1;
        sc.act[rb].vertex_top = sc.vtx[lm.vertex].next;
        sc.act[rb].top = sc.vtx[sc.act[rb].vertex_top.unwrap()].pt;
        sc.act[rb].local_min = Some(lm_idx);
        sc.act[rb].dx = get_dx(sc.act[rb].bot, sc.act[rb].top);
        // swap if needed
        let mut lb = lb;
        let mut rb = rb;
        if is_horz(sc, lb) {
            if sc.act[lb].dx == -f64::MAX {
                std::mem::swap(&mut lb, &mut rb);
            }
        } else if is_horz(sc, rb) {
            if sc.act[rb].dx == f64::MAX {
                std::mem::swap(&mut lb, &mut rb);
            }
        } else if sc.act[lb].dx < sc.act[rb].dx {
            std::mem::swap(&mut lb, &mut rb);
        }
        ...
        sc.act[rb].wind_cnt = sc.act[lb].wind_cnt;
        sc.act[rb].wind_cnt2 = sc.act[lb].wind_cnt2;
        insert_right_edge(sc, lb, rb);
        if contributing {
            add_local_min_poly(sc, lb, rb, sc.act[lb].bot, true);
            if !is_horz(sc, lb) {
                check_join_left(sc, lb, sc.act[lb].bot, false);
            }
        }
        while sc.act[rb].next_in_ael.is_some()
            && is_valid_ael_order(sc, sc.act[rb].next_in_ael.unwrap(), rb)
        {
        ...
        if is_horz(sc, rb) {
            sc.act[rb].next_in_sel = sc.sel;
            sc.sel = Some(rb);
        } else {
            check_join_right(sc, rb, sc.act[rb].bot, false);
            sc.push_scan(sc.act[rb].top.y);
        }
        if is_horz(sc, lb) {
            sc.act[lb].next_in_sel = sc.sel;
            sc.sel = Some(lb);
        } else {
            sc.push_scan(sc.act[lb].top.y);
        }
    }
}
...
fn do_maxima(sc: &mut Sc, e: usize, ct: i32) -> OptIdx {
    ...
    let max_pair = match get_maxima_pair(sc, e) {
        Some(mp) => mp,
        None => return sc.act[e].next_in_ael,
    };
    if is_joined(sc, e) {
        split(sc, e, sc.act[e].top);
    }
    if is_joined(sc, max_pair) {
        split(sc, max_pair, sc.act[max_pair].top);
    }
    ...
    if is_hot(sc, e) {
        add_local_max_poly(sc, e, max_pair, sc.act[e].top);
    }
    delete_from_ael(sc, max_pair);
    delete_from_ael(sc, e);
    if let Some(p) = prev_e {
        sc.act[p].next_in_ael
    } else {
        sc.actives
    }
}
...
fn do_top_of_scanbeam(sc: &mut Sc, y: i64, ct: i32) {
    ...
    while let Some(e) = e_opt {
        if sc.act[e].top.y == y {
            ...
            if is_maxima(sc, e) {
                e_opt = do_maxima(sc, e, ct);
                continue;
            }
            if is_hot(sc, e) {
                add_outpt(sc, e, sc.act[e].top);
            }
            update_edge_into_ael(sc, e, ct);
            if is_horz(sc, e) {
                sc.act[e].next_in_sel = sc.sel;
                sc.sel = Some(e);
            }
        } else {
            sc.act[e].curr_x = top_x(sc, e, y);
        }
        e_opt = sc.act[e].next_in_ael;
    }
}
...
fn do_split_op(sc: &mut Sc, or: usize, split_op: usize) {
    let prev_op = sc.opt[split_op].prev;
    let nn_op = sc.opt[sc.opt[split_op].next].next;
    sc.orc[or].pts = Some(prev_op);
    let ip = match get_seg_isect(
        sc.opt[prev_op].pt,
        sc.opt[split_op].pt,
        sc.opt[sc.opt[split_op].next].pt,
        sc.opt[nn_op].pt,
    ) {
        Some(p) => p,
        None => return,
    ...
    if ip == sc.opt[prev_op].pt || ip == sc.opt[nn_op].pt {
        sc.opt[nn_op].prev = prev_op;
        sc.opt[prev_op].next = nn_op;
    } else {
        let nop = sc.alloc_opt(ip, sc.opt[prev_op].outrec);
        sc.opt[nop].prev = prev_op;
        sc.opt[nop].next = nn_op;
        sc.opt[nn_op].prev = nop;
        sc.opt[prev_op].next = nop;
    }
    if area2.abs() >= 1.0 && (area2.abs() > area1.abs() || (area2 > 0.0) == (area1 > 0.0)) {
        let or_owner = sc.orc[or].owner;
        let nr = sc.alloc_orc();
        sc.orc[nr].owner = or_owner;
        let split_next = sc.opt[split_op].next;
        sc.opt[split_op].outrec = nr;
        sc.opt[split_next].outrec = nr;
        let nop = sc.alloc_opt(ip, nr);
        sc.opt[nop].prev = split_next;
        sc.opt[nop].next = split_op;
        sc.orc[nr].pts = Some(nop);
        sc.opt[split_op].prev = nop;
        sc.opt[split_next].next = nop;
    }
}

fn fix_self_intersects(sc: &mut Sc, or: usize) {
    let mut op2 = match sc.orc[or].pts {
        Some(p) => p,
        None => return,
    };
    loop {
        if sc.opt[op2].prev == sc.opt[sc.opt[op2].next].next {
            break;
        }
        if segs_intersect(
            sc.opt[sc.opt[op2].prev].pt,
            sc.opt[op2].pt,
            sc.opt[sc.opt[op2].next].pt,
            sc.opt[sc.opt[sc.opt[op2].next].next].pt,
        ) {
            let pts = sc.orc[or].pts.unwrap();
            if op2 == pts || sc.opt[op2].next == pts {
                sc.orc[or].pts = Some(sc.opt[pts].prev);
            }
            do_split_op(sc, or, op2);
            if sc.orc[or].pts.is_none() {
                break;
            }
            op2 = sc.orc[or].pts.unwrap();
            continue;
        }
        op2 = sc.opt[op2].next;
        if op2 == sc.orc[or].pts.unwrap_or(usize::MAX) {
            break;
        }
    }
}
...
fn clean_collinear(sc: &mut Sc, or_idx: usize) {
    let mut or = or_idx;
    while sc.orc[or].pts.is_none() {
        match sc.orc[or].owner {
            Some(o) => or = o,
            None => return,
        };
    }
    if !valid_closed(sc, sc.orc[or].pts.unwrap()) {
        sc.orc[or].pts = None;
        return;
    }
    ...
    loop {
        let prev = sc.opt[op2].prev;
        let next = sc.opt[op2].next;
        if is_collinear(sc.opt[prev].pt, sc.opt[op2].pt, sc.opt[next].pt)
            && (sc.opt[op2].pt == sc.opt[prev].pt
                || sc.opt[op2].pt == sc.opt[next].pt
                || dot3(sc.opt[prev].pt, sc.opt[op2].pt, sc.opt[next].pt) < 0.0)
        {
            if op2 == sc.orc[or].pts.unwrap() {
                sc.orc[or].pts = Some(prev);
            }
            op2 = dispose_outpt(sc, op2);
            if !valid_closed(sc, op2) {
                sc.orc[or].pts = None;
                return;
            }
            start_op = op2;
...
fn execute_internal(sc: &mut Sc, ct: i32) -> bool {
    sc.locmin.sort_by(|a, b| {
        let ay = sc.vtx[a.vertex].pt.y;
        let by = sc.vtx[b.vertex].pt.y;
        if by != ay {
            by.cmp(&ay)
        } else {
            sc.vtx[b.vertex].pt.x.cmp(&sc.vtx[a.vertex].pt.x).reverse()
        }
    });
    for lm in &sc.locmin {
        sc.scan.push(sc.vtx[lm.vertex].pt.y);
    }
    // build heap
    let n = sc.scan.len();
    for i in (0..n / 2).rev() {
        ...
        loop {
            let (l, r) = (2 * k + 1, 2 * k + 2);
            let mut m = k;
            if l < n && sc.scan[l] > sc.scan[m] {
                m = l;
            }
            if r < n && sc.scan[r] > sc.scan[m] {
                m = r;
            }
            if m == k {
                break;
    ...
    while sc.ok {
        ...
        // pop horz
        while sc.sel.is_some() {
            let e = sc.sel.unwrap();
            sc.sel = sc.act[e].next_in_sel;
            do_horizontal(sc, e, ct);
        }
        if !sc.hsegs.is_empty() {
            convert_horz_segs_to_joins(sc);
            sc.hsegs.clear();
        }
        sc.bot_y = y;
        y = match sc.pop_scan() {
        ...
        if sc.ok && build_intersect_list(sc, y) {
            process_intersect_list(sc, ct);
            sc.isnodes.clear();
        }
...
//
// For convex CCW inputs (a box's top, bottom and side faces are all convex
// quads), the full Vatti scanline is enormous
...
pub fn boolean_op(a: &Polyline, b: &Polyline, clip_type: i32) -> Vec<Polyline> {
    ...
    if na >= 2 {
        let dx = ca[(na - 1) * 3] - ca[0];
        let dy = ca[(na - 1) * 3 + 1] - ca[1];
        if dx * dx + dy * dy < 1e-20 {
            na -= 1;
        }
    }
    if nb >= 2 {
        let dx = cb[(nb - 1) * 3] - cb[0];
        let dy = cb[(nb - 1) * 3 + 1] - cb[1];
        if dx * dx + dy * dy < 1e-20 {
            nb -= 1;
        }
    }
...
fn boolean_op_inner(
    ...
    if (na as i64) * (nb as i64) <= 400 {
        let cvt = |c: &[f64], i: usize| -> Pt {
            Pt {
                x: (c[i * 3] * bool_scale).round() as i64,
                y: (c[i * 3 + 1] * bool_scale).round() as i64,
            }
        };
        ...
        if amaxx < bminx || bmaxx < aminx || amaxy < bminy || bmaxy < aminy {
            let a_in_b = pip_i(va[0], &vb);
            let b_in_a = pip_i(vb[0], &va);
            if clip_type == 0 {
                return if a_in_b {
                    vec![a.clone()]
                } else if b_in_a {
        ...
        if !any_cross {
            ...
            let mut ca_cen = Pt { x: 0, y: 0 };
            let mut cb_cen = Pt { x: 0, y: 0 };
            for i in 0..na {
                ca_cen.x += va[i].x;
                ca_cen.y += va[i].y;
            }
        ...
        // disjoint-AABB or no-crossing. For convex CCW quads,
        // Sutherland-Hodgman in float64 skips the
        ...
        let base_a = 0;
        {
            ...
            sc.vtx[prev].next = Some(base_a);
            sc.vtx[base_a].prev = Some(prev);
            // locmin detection
            let mut pv = sc.vtx[base_a].prev.unwrap();
            while pv != base_a && sc.vtx[pv].pt.y == sc.vtx[base_a].pt.y {
                pv = sc.vtx[pv].prev.unwrap();
            }
            if pv != base_a {
                let mut going_up = sc.vtx[pv].pt.y > sc.vtx[base_a].pt.y;
                let going_up0 = going_up;
                pv = base_a;
                let mut cv = sc.vtx[base_a].next.unwrap();
                while cv != base_a {
                    if sc.vtx[cv].pt.y > sc.vtx[pv].pt.y && going_up {
                        sc.vtx[pv].flags |= VF_LOCAL_MAX;
                        going_up = false;
        ...
        // wait, we already allocated va vertices above. Let me fix: the vb vertices start at na
        {
            ...
            sc.vtx[prev].next = Some(base_b);
            sc.vtx[base_b].prev = Some(prev);
            let mut pv = sc.vtx[base_b].prev.unwrap();
            while pv != base_b && sc.vtx[pv].pt.y == sc.vtx[base_b].pt.y {
                pv = sc.vtx[pv].prev.unwrap();
            }
            if pv != base_b {
                let mut going_up = sc.vtx[pv].pt.y > sc.vtx[base_b].pt.y;
                let going_up0 = going_up;
                pv = base_b;
                let mut cv = sc.vtx[base_b].next.unwrap();
                while cv != base_b {
                    if sc.vtx[cv].pt.y > sc.vtx[pv].pt.y && going_up {
                        sc.vtx[pv].flags |= VF_LOCAL_MAX;
                        going_up = false;
    ...
    if !execute_internal(sc, clip_type) {
        return vec![];
    }
    ...
    for &or_idx in &orc_indices {
        ...
        if sc.opt[op].next == op || sc.opt[op].next == sc.opt[op].prev {
            continue;
        }
        if very_small_tri(sc, op) {
            continue;
        }
        let mut coords = Vec::new();
        let mut o = sc.opt[op].next;
        let mut last = sc.opt[o].pt;
        coords.push(last.x as f64 * bool_inv_scale);
        coords.push(last.y as f64 * bool_inv_scale);
        coords.push(0.0);
        o = sc.opt[o].next;
...
pub fn compute_count(a: &Polyline, b: &Polyline, clip_type: i32) -> i32 {
    ...
    SCRATCH.with(|cell| {
        ...
        let (va_head, aminx, amaxx, aminy, amaxy) =
            add_path_from_coords(&mut sc, ca, na, 0, bool_scale);
        let (vb_head, bminx, bmaxx, bminy, bmaxy) =
            add_path_from_coords(&mut sc, cb, nb, 1, bool_scale);
        if va_head.is_none() || vb_head.is_none() {
            return 0;
        }
        if amaxx < bminx || bmaxx < aminx || amaxy < bminy || bmaxy < aminy {
        ...
        if !execute_internal(&mut sc, clip_type) {
            return 0;
        }
