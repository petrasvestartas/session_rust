...
impl RemeshNurbsSurfaceAdaptive {
    ...
    pub fn mesh(&self) -> Mesh {
            ...
            ($nodes:expr) => {{
                while let Some(idx) = subdivide_stack.pop() {
                    ...
                    if min_edge_length > 0.0 {
                        ...
                        if me < min_edge_length * min_edge_length {
                            continue;
                        }
                    }
                    ...
                    if nvalid(&p.c[0]) && nvalid(&p.c[1]) && ndiff(&p.c[0], &p.c[1]) > norm_tol {
                        split_u = true;
                    }
                    if nvalid(&p.c[2]) && nvalid(&p.c[3]) && ndiff(&p.c[2], &p.c[3]) > norm_tol {
                        split_u = true;
                    }
                    if nvalid(&p.c[1]) && nvalid(&p.c[2]) && ndiff(&p.c[1], &p.c[2]) > norm_tol {
                        split_v = true;
                    }
                    if nvalid(&p.c[3]) && nvalid(&p.c[0]) && ndiff(&p.c[3], &p.c[0]) > norm_tol {
                        split_v = true;
                    }

                    let mut s_mid = Corner::default();
                    let mut n_mid = Corner::default();
                    let mut w_mid = Corner::default();
                    let mut e_mid = Corner::default();
                    let mut have_sn = false;
                    let mut have_we = false;

                    if !split_u {
                        s_mid = eval_corner(um, p.v0);
                        n_mid = eval_corner(um, p.v1);
                        have_sn = true;
                        let s_lin = pmid(&p.c[0], &p.c[1]);
                        let n_lin = pmid(&p.c[3], &p.c[2]);
                        if pdist2(&s_mid, &s_lin) > chord_tol * chord_tol {
                            split_u = true;
                        }
                        if pdist2(&n_mid, &n_lin) > chord_tol * chord_tol {
                            split_u = true;
                        }
                        if nvalid(&s_mid) && nvalid(&p.c[0]) && ndiff(&s_mid, &p.c[0]) > norm_tol {
                            split_u = true;
                        }
                        if nvalid(&n_mid) && nvalid(&p.c[3]) && ndiff(&n_mid, &p.c[3]) > norm_tol {
                            split_u = true;
                        }
                    }
                    if !split_v {
                        w_mid = eval_corner(p.u0, vm);
                        e_mid = eval_corner(p.u1, vm);
                        have_we = true;
                        let w_lin = pmid(&p.c[0], &p.c[3]);
                        let e_lin = pmid(&p.c[1], &p.c[2]);
                        if pdist2(&w_mid, &w_lin) > chord_tol * chord_tol {
                            split_v = true;
                        }
                        if pdist2(&e_mid, &e_lin) > chord_tol * chord_tol {
                            split_v = true;
                        }
                        if nvalid(&w_mid) && nvalid(&p.c[0]) && ndiff(&w_mid, &p.c[0]) > norm_tol {
                            split_v = true;
                        }
                        if nvalid(&e_mid) && nvalid(&p.c[1]) && ndiff(&e_mid, &p.c[1]) > norm_tol {
                            split_v = true;
                        }
                    }
                    ...
                    if !split_u && !split_v {
                        let degenerate = (0..4)
                            .any(|ci| pdist2(&p.c[ci], &p.c[(ci + 1) % 4]) < chord_tol * chord_tol);
                        if !degenerate {
                            let twist_tol2 = 4.0 * chord_tol * chord_tol;
                            for d in 0..2 {
                                let mid = pmid(&p.c[d], &p.c[d + 2]);
                                if pdist2(&p.c[4], &mid) > twist_tol2 {
                                    split_u = true;
                                    split_v = true;
                                }
                            }
                    ...
                    if max_edge_length > 0.0 {
                        ...
                        if pdist2(&p.c[0], &p.c[1]) > me || pdist2(&p.c[2], &p.c[3]) > me {
                            split_u = true;
                        }
                        if pdist2(&p.c[1], &p.c[2]) > me || pdist2(&p.c[3], &p.c[0]) > me {
                            split_v = true;
                        }
                    }
                    ...
                    if split_u && split_v {
                        ...
                        $nodes.resize_with($nodes.len() + 4, || Node {
                            u0: 0.0,
                            v0: 0.0,
                            u1: 0.0,
                            v1: 0.0,
                        ...
                        subdivide_stack.extend_from_slice(&[
                            b as usize,
                            b as usize + 1,
                            b as usize + 2,
                            b as usize + 3,
                        ...
                        $nodes.resize_with($nodes.len() + 2, || Node {
                            u0: 0.0,
                            v0: 0.0,
                            u1: 0.0,
                            v1: 0.0,
        ...
        let norm_u_fn = |u: f64| -> f64 {
            if closed_u && (u - usp[usp.len() - 1]).abs() < 1e-10 {
                usp[0]
            } else {
                u
            }
        ...
        let norm_v_fn = |v: f64| -> f64 {
            if closed_v && (v - vsp[vsp.len() - 1]).abs() < 1e-10 {
                vsp[0]
            } else {
                v
            }
        ...
        for vec in by_v.values_mut() {
            vec.sort();
            vec.dedup();
        }
        for vec in by_u.values_mut() {
            vec.sort();
            vec.dedup();
        }
        ...
        let find_h_mids =
            |u_start: f64, u_end: f64, v_const: f64, by_v: &BTreeMap<i64, Vec<i64>>| -> Vec<f64> {
                let qv = qk(norm_v_fn(v_const));
                let vec = match by_v.get(&qv) {
                    Some(v) => v,
                    None => return Vec::new(),
                };
        ...
        let find_v_mids =
            |u_const: f64, v_start: f64, v_end: f64, by_u: &BTreeMap<i64, Vec<i64>>| -> Vec<f64> {
                let qu = qk(norm_u_fn(u_const));
                let vec = match by_u.get(&qu) {
                    Some(v) => v,
                    None => return Vec::new(),
                };
            ...
            ($u:expr, $v:expr) => {{
                ...
                if sing_v0 && (v - vsp[0]).abs() < 1e-10 {
                    ...
                    if let Some(&vi) = vtx_map.get(&key) {
                        ...
                        let c = match key_corner.get(&key) {
                            Some(&corner) => corner,
                            None => eval_corner(un, vn),
                        };
        ...
        for ni in 0..nodes.len() {
            ...
            for u in find_h_mids(nd.u0, nd.u1, nd.v0, &by_v) {
                poly.push(get_vtx!(u, nd.v0));
            }
            poly.push(get_vtx!(nd.u1, nd.v0));
            for v in find_v_mids(nd.u1, nd.v0, nd.v1, &by_u) {
                poly.push(get_vtx!(nd.u1, v));
            }
            poly.push(get_vtx!(nd.u1, nd.v1));
            for u in find_h_mids(nd.u1, nd.u0, nd.v1, &by_v) {
                poly.push(get_vtx!(u, nd.v1));
            }
            poly.push(get_vtx!(nd.u0, nd.v1));
            for v in find_v_mids(nd.u0, nd.v1, nd.v0, &by_u) {
                poly.push(get_vtx!(nd.u0, v));
            }

            // Remove consecutive duplicates and wrap-around duplicate
            let mut deduped: Vec<usize> = Vec::new();
            for vi in &poly {
                if deduped.last() != Some(vi) {
                    deduped.push(*vi);
                }
            }
            ...
            if n == 3 {
                ...
                let d02 = (p0[0] - p2[0]) * (p0[0] - p2[0])
                    + (p0[1] - p2[1]) * (p0[1] - p2[1])
                    + (p0[2] - p2[2]) * (p0[2] - p2[2]);
                let d13 = (p1[0] - p3[0]) * (p1[0] - p3[0])
                    + (p1[1] - p3[1]) * (p1[1] - p3[1])
                    + (p1[2] - p3[2]) * (p1[2] - p3[2]);
                if d02 <= d13 {
                    result.add_face(vec![poly[0], poly[1], poly[2]], None);
        ...
        let nv = if result.vertex.is_empty() {
            0
        } else {
            *result.vertex.keys().max().unwrap() + 1
        };
