...
fn split_flags(q: &Quadtree, p: &Node, mids: &[Corner; 4]) -> (bool, bool) {
    ...
    if !split_u || !split_v {
        let (curved_u, curved_v) = curved(q.s, p, q.chord_tol);
...
fn subdivide(q: &mut Quadtree, root: usize) {
    ...
    while top > 0 {
        ...
        let (split_u, split_v) = split_flags(q, &p, &mids);
...
fn between(lines: &BTreeMap<i64, Vec<i64>>, line: i64, t0: f64, t1: f64) -> Vec<f64> {
    ...
    let found = match lines.get(&line) {
        Some(found) => found,
        None => return result,
    };
...
fn vertex_at(q: &mut Quadtree, u: f64, v: f64) -> usize {
    ...
    let key: Key = (quantize(uw), quantize(vw));
...
fn leaf_polygon(q: &mut Quadtree, nd: &Node) -> Vec<usize> {
    ...
    poly.push(vertex_at(q, nd.u0, nd.v0));
...
fn add_leaf_faces(q: &mut Quadtree, nd: &Node) {
    ...
    q.corners
        .entry((quantize(cu), quantize(cv)))
        .or_insert_with(|| nd.c[4].clone());
...
impl RemeshNurbsSurfaceAdaptive {
    /// Constructs over a surface with the default tolerances.
    ...
    /// Returns the largest normal turn in degrees.
    pub fn get_max_angle(&self) -> f64 {
        self.max_angle
    }

    /// Returns the longest cell edge.
    pub fn get_max_edge_length(&self) -> f64 {
        self.max_edge_length
    }

    /// Returns the shortest cell edge still split.
    pub fn get_min_edge_length(&self) -> f64 {
        self.min_edge_length
    }

    /// Returns the largest chord height.
