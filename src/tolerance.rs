...
impl Tolerance {
    ...
    /// Default tolerance values (loosened from f64 originals to fit f64 precision).
    /// ABSOLUTE just above f64 machine epsilon (≈1.2e-7); algorithms that need
    /// tighter checking should set per-call tolerance explicitly.
    pub const ABSOLUTE: f64 = 1e-5;
    pub const RELATIVE: f64 = 1e-5;
    pub const ANGULAR: f64 = 1e-5;
    ...
    pub const ZERO_TOLERANCE: f64 = 1e-7;
    ...
    pub fn key(&self, xyz: [f64; 3], precision: i32) -> String {
        let precision = if precision == -999 {
            self.precision()
        } else {
            precision
        };
...
impl GlobalTolerance {
    ...
    pub fn key(&self, xyz: [f64; 3], precision: i32) -> String {
        self.inner.read().key(xyz, precision)
    }
    pub fn key_xy(&self, xy: [f64; 2], precision: i32) -> String {
        self.inner.read().key_xy(xy, precision)
    }
    pub fn format_number(&self, number: f64, precision: i32) -> String {
