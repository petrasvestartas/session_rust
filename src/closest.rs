...
impl Closest {
    ...
    pub fn curve_curve(curve0: &NurbsCurve, curve1: &NurbsCurve) -> (f64, f64, f64) {
        ...
        let p0: Vec<Point> = (0..=n0)
            .map(|i| curve0.point_at(u0 + (u1 - u0) * i as f64 / n0 as f64))
            .collect();
        let p1: Vec<Point> = (0..=n1)
            .map(|j| curve1.point_at(v0 + (v1 - v0) * j as f64 / n1 as f64))
            .collect();
        ...
        for i in 0..=n0 {
            for j in 0..=n1 {
                let dx = p0[i][0] - p1[j][0];
                let dy = p0[i][1] - p1[j][1];
                let dz = p0[i][2] - p1[j][2];
        ...
        for _ in 0..64 {
            ...
            if du.abs() > (u1 - u0) * 0.5 {
                du = du.signum() * (u1 - u0) * 0.5;
            }
            if dv.abs() > (v1 - v0) * 0.5 {
                dv = dv.signum() * (v1 - v0) * 0.5;
            }
        ...
        for _ in 0..max_iterations {
            ...
            if let Some(pt) = surface.point_at(u, v) {
                ...
                if du_step.abs() > max_step {
                    du_step = du_step.signum() * max_step;
                }
                if dv_step.abs() > max_step {
                    dv_step = dv_step.signum() * max_step;
                }
    ...
    pub fn surface_curve(
        ...
        let uv_to_3d_u = pmid.distance(
            &surface
                .point_at(wu_probe, mv)
                .unwrap_or(Point::new(0.0, 0.0, 0.0)),
            None,
        ) / du;
        let uv_to_3d_v = pmid.distance(
            &surface
                .point_at(mu, wv_probe)
                .unwrap_or(Point::new(0.0, 0.0, 0.0)),
            None,
        ) / dv;
        ...
        for i in 0..=n0 {
            ...
            let (uu, vv, rd) = if i == 0 {
                ...
                let (mut ru, mut rv, mut rd) =
                    invert_near(&pt, wrap_u(prev[1]), wrap_v(prev[2]), wu, wv);
                if rd > reject_tol {
    ...
    pub fn mesh_point_aabb(mesh: &Mesh, test_point: &Point) -> (Point, usize, f64) {
        ...
        fn dfs(
            ...
            tri_face_idx: &[usize],
            sorted_face_keys: &[usize],
            test_point: &Point,
            best_point: &mut Point,
            best_face_key: &mut usize,
            best_dist: &mut f64,
        ) {
            let (ref aabb, right, obj) = nodes[ni];
            if aabb_min_dist(aabb, test_point) >= *best_dist {
                return;
            }
            if obj >= 0 {
    ...
    pub fn pointcloud_point_kdtree(cloud: &PointCloud, test_point: &Point) -> (Point, usize, f64) {
        ...
        let pts: Vec<Point> = (0..cloud.point_count())
            .map(|i| cloud.get_point(i))
            .collect();
    ...
    pub fn polylines_closest(polylines: &[Polyline], threshold: f64) -> Vec<(usize, usize)> {
        ...
        for i in 0..polylines.len() {
            ...
            for j in candidates {
                ...
                let dist = pts_a
                    .iter()
                    .map(|pt| Self::polyline_point(&polylines[j], pt).2)
    ...
    pub fn boxes_closest(boxes: &[crate::aabb::AABB], threshold: f64) -> Vec<(usize, usize)> {
        ...
        let raw_orig: Vec<[f64; 6]> = boxes
            .iter()
            .map(|b| [b.cx, b.cy, b.cz, b.hx, b.hy, b.hz])
            .collect();
        let raw_inf: Vec<[f64; 6]> = boxes
            .iter()
            .map(|b| {
                [
                    b.cx,
