...
const STACK_SIZE: usize = 64; // Explicit stack depth, covers any binary tree over usize.
const NULL_IDX: usize = usize::MAX; // Missing child marker.

/// A split node: point index, axis and children.
struct Node {
    idx: usize,   // Point index.
    axis: usize,  // Split axis (0=x, 1=y, 2=z).
    left: usize,  // Left child node or NULL_IDX.
    right: usize, // Right child node or NULL_IDX.
}

/// A pending index range on the build stack.
#[derive(Clone, Copy)]
struct Range {
    lo: usize,     // Range start.
    hi: usize,     // Range end, exclusive.
    depth: usize,  // Depth from the root.
    parent: usize, // Parent node or NULL_IDX.
    is_left: bool, // True when this range is the parent's left child.
}
...
pub struct SpatialKDTree {
    points: Vec<Point>, // Indexed points.
