...
mod picking_tests {
    ...
    fn test_ray_cast_far_box_hit() {
        ...
        let dz = 1000.0 - 1500.0_f64;
        let dir = Vector::new(dx, dy, dz);
        let hits = s.ray_cast(&cam_mm, &dir, 1.0);
        assert!(
            !hits.is_empty(),
            "ray from ~camera toward (2000,2000,1000) should hit box_blue-equivalent"
        );
    }
    ...
    fn test_ray_cast_far_box_green() {
        ...
        let cam_mm = Point::new(1840.0, -1840.0, 1500.0);
        let dx = -2500.0 - 1840.0_f64;
        let dy = 1000.0 - (-1840.0_f64);
        let dz = 2000.0 - 1500.0_f64;
        let dir = Vector::new(dx, dy, dz);
        let hits = s.ray_cast(&cam_mm, &dir, 1.0);
        assert!(
            !hits.is_empty(),
            "ray toward box_green-equivalent should hit"
        );
    }
    ...
    fn test_ray_cast_per_object_selectability() {
        ...
        for &(name, (cx, cy, cz), r) in cases {
            ...
            assert!(
                !hits.is_empty(),
                "FAIL: {} at ({},{},{}) r={} — ray_cast returned no hits",
                name,
                cx,
                cy,
                cz,
    ...
    fn test_bvh_world_size_covers_far_objects() {
        ...
        for (i, obb) in obbs.iter().enumerate() {
            ...
            assert!(
                obb.center[0].abs() <= half,
                "OBB {} center.x {} outside world_size {}",
                i,
                obb.center[0],
                world_size
            );
            assert!(
                obb.center[1].abs() <= half,
                "OBB {} center.y {} outside world_size {}",
                i,
                obb.center[1],
                world_size
            );
            assert!(
                obb.center[2].abs() <= half,
                "OBB {} center.z {} outside world_size {}",
                i,
                obb.center[2],
                world_size
            );
    ...
    fn test_viewer_ray_simple_z_camera() {
        let eye_m = [0.0f64, 0.0, 10.0];
        let tgt_m = [0.0f64, 0.0, 0.0];
    ...
    fn test_viewer_ray_default_camera() {
        let eye_m = [1.836f64, -1.836, 1.5];
        let tgt_m = [0.0f64, 0.0, 0.0];
        let up = [-0.354f64, 0.354, 0.866];
        let near = 3.0_f64 * 0.001;
        let view = build_view_matrix(eye_m, tgt_m, up);
        let proj = build_proj_matrix(Tolerance::PI / 3.0, 16.0 / 9.0, near, 100_000.0);
        let vp = (1920.0f64, 1080.0);
