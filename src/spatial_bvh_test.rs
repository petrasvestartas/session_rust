...
pub fn run_bvh_constructor() -> TestResult {
    MINI_TEST!("Constructor", {
        ...
        let boxes = vec![
            OBB::from_aabb(&AABB::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
            OBB::from_aabb(&AABB::new(2.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
            OBB::from_aabb(&AABB::new(20.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
        ];
...
pub fn run_bvh_creation() -> TestResult {
    MINI_TEST!("Creation", {
        ...
        let bvh = SpatialBVH::with_world_size(100.0);
...
pub fn run_bvh_build_single() -> TestResult {
    MINI_TEST!("Build Single", {
        ...
        let boxes = vec![OBB::from_aabb(&AABB::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0))];
        let bvh = SpatialBVH::from_boxes(&boxes, 100.0);
...
pub fn run_bvh_build_multiple() -> TestResult {
    MINI_TEST!("Build Multiple", {
        ...
        let boxes = vec![
            OBB::from_aabb(&AABB::new(-10.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
            OBB::from_aabb(&AABB::new(10.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
            OBB::from_aabb(&AABB::new(0.0, 10.0, 0.0, 1.0, 1.0, 1.0)),
        ];
        let bvh = SpatialBVH::from_boxes(&boxes, 100.0);
...
pub fn run_bvh_aabb_intersect() -> TestResult {
    MINI_TEST!("Aabb Intersect", {
        ...
        let bvh = SpatialBVH::with_world_size(100.0);
        let bbox1 = OBB::from_aabb(&AABB::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0));
        let bbox2 = OBB::from_aabb(&AABB::new(0.5, 0.0, 0.0, 1.0, 1.0, 1.0));
        let bbox3 = OBB::from_aabb(&AABB::new(10.0, 0.0, 0.0, 1.0, 1.0, 1.0));

        MINI_CHECK!(bvh.obb_intersect(&bbox1, &bbox2));
        MINI_CHECK!(!bvh.obb_intersect(&bbox1, &bbox3));
        MINI_CHECK!(bvh.aabb_intersect(
            &AABB::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0),
            &AABB::new(1.5, 0.0, 0.0, 1.0, 1.0, 1.0)
        ));
    })
}
...
pub fn run_bvh_check_all_collisions() -> TestResult {
    MINI_TEST!("Check All Collisions", {
        ...
        let boxes = vec![
            OBB::from_aabb(&AABB::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
            OBB::from_aabb(&AABB::new(0.5, 0.0, 0.0, 1.0, 1.0, 1.0)),
            OBB::from_aabb(&AABB::new(10.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
        ];
        let bvh = SpatialBVH::from_boxes(&boxes, 100.0);
        let result = bvh.check_all_collisions(&boxes);
        let collisions = &result.0;
        let colliding_indices = &result.1;
...
pub fn run_bvh_nearest_neighbors() -> TestResult {
    MINI_TEST!("Nearest Neighbors", {
        ...
        let bvh = SpatialBVH::from_boxes(&boxes, 100.0);
        let n0 = bvh.nearest_neighbors(0, &boxes, 1.2);

        MINI_CHECK!(n0.len() == 1);
        MINI_CHECK!(n0[0] == 1);

        let n2 = bvh.nearest_neighbors(2, &boxes, 1.2);

        MINI_CHECK!(n2.is_empty());

        let n2_wide = bvh.nearest_neighbors(2, &boxes, 10.0);
...
pub fn run_bvh_merge_aabb() -> TestResult {
    MINI_TEST!("Merge Aabb", {
        ...
        let bvh = SpatialBVH::with_world_size(100.0);
        let bbox1 = OBB::from_aabb(&AABB::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0));
        let bbox2 = OBB::from_aabb(&AABB::new(5.0, 0.0, 0.0, 1.0, 1.0, 1.0));
...
pub fn run_bvh_fixed_100_boxes() -> TestResult {
    MINI_TEST!("Fixed 100 Boxes", {
        ...
        for corner in &corners {
            let lo = Point::new(corner[0], corner[1], corner[2]);
            let hi = Point::new(corner[3], corner[4], corner[5]);
            boxes.push(OBB::from_aabb(&AABB::from_points(&[lo, hi], 0.0)));
        }

        let bvh = SpatialBVH::from_boxes(&boxes, 100.0);
        let mut pairs = bvh.check_all_collisions(&boxes).0;
...
pub fn run_bvh_query_aabb() -> TestResult {
    MINI_TEST!("Query Aabb", {
        ...
        let boxes = vec![
            OBB::from_aabb(&AABB::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
            OBB::from_aabb(&AABB::new(5.0, 0.0, 0.0, 1.0, 1.0, 1.0)),
            OBB::from_aabb(&AABB::new(0.0, 5.0, 0.0, 1.0, 1.0, 1.0)),
        ];
        let bvh = SpatialBVH::from_boxes(&boxes, 100.0);
        let hits = bvh.query_obb(&OBB::from_aabb(&AABB::new(0.0, 0.0, 0.0, 0.5, 0.5, 0.5)));

        MINI_CHECK!(hits.contains(&0));
        MINI_CHECK!(!hits.contains(&1));
        MINI_CHECK!(!hits.contains(&2));

        let hits_all = bvh.query_aabb(&AABB::new(2.5, 2.5, 0.0, 5.0, 5.0, 2.0));
...
pub fn run_bvh_build_from_boxes() -> TestResult {
    MINI_TEST!("Build From Boxes", {
        ...
        let pairs = bvh.check_all_collisions(&boxes).0;
...
pub fn run_bvh_build_from_aabbs() -> TestResult {
    MINI_TEST!("Build From Aabbs", {
        ...
        let hits = bvh.query_aabb(&AABB::new(0.0, 0.0, 0.0, 2.0, 2.0, 2.0));
...
pub fn run_bvh_find_collisions() -> TestResult {
    MINI_TEST!("Find Collisions", {
        ...
        let bvh = SpatialBVH::from_boxes(&boxes, 100.0);
        let found0 = bvh.find_collisions(0, &boxes[0], &boxes);
        let found2 = bvh.find_collisions(2, &boxes[2], &boxes);

        MINI_CHECK!(found0.0 == vec![1]);
        MINI_CHECK!(found0.1 > 0);
        MINI_CHECK!(found2.0.is_empty());
    })
}
...
REGISTER_MINI_TEST!(
    "SpatialBVH",
    "Ray Cast",
    crate::spatial_bvh_test::run_bvh_ray_cast
);
