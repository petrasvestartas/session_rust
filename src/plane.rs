...
impl Plane {
    /// Constructs from an origin and two axes; x is normalized, y is made orthogonal to x, z = x × y.
    pub fn new(point: Point, x_axis: Vector, y_axis: Vector) -> Self {
        Self::with_name(point, x_axis, y_axis, "my_plane")
    }

    /// Constructs from an origin, two axes and a name; x is normalized, y is made orthogonal to x, z = x × y.
    ...
    /// Returns plane equation coefficient a.
    pub fn a(&self) -> f64 {
        self._a
    }

    /// Returns plane equation coefficient b.
    pub fn b(&self) -> f64 {
        self._b
    }

    /// Returns plane equation coefficient c.
    pub fn c(&self) -> f64 {
        self._c
    }

    /// Returns plane equation coefficient d.
    pub fn d(&self) -> f64 {
        self._d
    }

    /// Recomputes a, b, c, d from z_axis and origin.
    ...
    /// Constructs from a frame taken as given, no normalization.
    ...
    /// Constructs the plane through point with normal as z axis.
    ...
    /// Constructs the plane through the first three points, x axis along the first edge.
    ...
    /// Constructs the least-squares plane through points by power-iteration PCA.
    ...
    /// Constructs the plane with x axis from point1 to point2.
    ...
    /// Constructs an all-zero frame that fails is_valid().
    pub fn invalid() -> Self {
        Self::from_frame(
    ...
    /// Returns whether the frame is non-zero.
    ...
    /// Swaps x and y and flip z in place.
    ...
    /// Rotates x and y around z in place.
    ...
    /// Returns whether x × y points along z.
    pub fn is_right_hand(&self) -> bool {
        self._x_axis.cross(&self._y_axis).dot(&self._z_axis) > 0.999
    }

    /// Returns whether the normals are parallel (can_be_flipped) or exactly opposite (!can_be_flipped).
    ...
    /// Returns whether each origin lies on the other plane.
    ...
    /// Returns whether the planes share direction and position.
    ...
    /// Returns is_coplanar from origin and normal pairs without building planes; tolerance < 0 uses APPROXIMATION.
    ...
    /// Returns a copy moved along z by distance.
    ...
    /// Returns the orthogonal projection of p onto the plane.
    ...
    /// Returns the squared distance from p to the plane.
    ...
    /// Returns the canonical in-plane axis from the normal alone: zero the smallest normal coordinate, negate-swap the other two.
    ...
    /// Returns z × base1 at unit length.
    ...
    /// Returns the square outline of side scale plus the three axes as polylines.
    ...
    /// Constructs from a protobuf message.
    ...
    /// Returns "Plane(name, ox, oy, oz, zx, zy, zz, Color(...))".
    ...
    /// Returns a GPU-ready column-major [f32; 16] frame matrix (x, y, z, origin columns), the f64 to f32 boundary for wgpu upload.
