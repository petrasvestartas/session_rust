...
impl Matrix {
    ...
    /// # Panics
    ///
    /// Panics if either dimension exceeds the shared API range or their product
    /// overflows `usize`.
    ...
    /// # Panics
    ///
    /// Panics if the dimensions are too large.
    ...
    /// # Panics
    ///
    /// Panics if the dimensions are too large or do not match `data`.
    ...
    /// # Panics
    ///
    /// Panics if row lengths differ or the dimensions are too large.
    ...
    /// # Panics
    ///
    /// Panics if column lengths differ or the dimensions are too large.
    ...
    /// Adds an equal-sized matrix through the legacy named API.
    ///
    /// # Panics
    ///
    /// Panics if the dimensions differ.
    ...
    /// # Panics
    ///
    /// Panics if this column count differs from the other row count.
    ...
    pub fn lu_decompose(&self) -> (Matrix, Matrix, Matrix) {
        ...
        let (l, u, p, _swaps) = self._lu_internal();
        (l, u, p)
    }
    ...
    pub fn determinant(&self) -> f64 {
        ...
        let (_l, u, _p, swaps) = self._lu_internal();
    ...
    ///
    /// Returns `None` if the shapes are invalid or the matrix is singular.
    ...
    ///
    /// Returns `None` if the matrix is non-square or not positive definite.
    ...
    /// # Errors
    ///
    /// Returns an error for malformed JSON or an invalid matrix shape.
    ...
    /// # Panics
    ///
    /// Panics for malformed JSON or an invalid matrix shape.
    ...
    /// # Errors
    ///
    /// Returns an error if reading, parsing, or shape validation fails.
    ...
    /// # Errors
    ///
    /// Returns an error for negative dimensions, oversized dimensions, or a
    /// data length that does not match the shape.
    ...
    /// # Errors
    ///
    /// Returns an error if decoding or shape validation fails.
    ...
    /// # Panics
    ///
    /// Panics if encoding or file writing fails.
    ...
    /// # Panics
    ///
    /// Panics if reading, decoding, or shape validation fails.
