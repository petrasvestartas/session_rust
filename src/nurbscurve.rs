...
impl NurbsCurve {
    ...
    pub fn create_fitted(
        ...
        if is_periodic {
            ...
            for k in 0..n {
                let span = nurbsknot::find_span(order, cv_count, &nurbsknots, params[k]);
        ...
        for k in 1..(m - 1) {
            let span = nurbsknot::find_span(order, num_cvs, &nurbsknots, params[k]);
            let basis = nurbsknot::eval_basis(order, &nurbsknots, span, params[k]);
    ...
    pub fn is_linear(&self, tolerance: Option<f64>) -> bool {
        ...
        for i in 1..(self.m_cv_count - 1) {
            ...
            if line_vec.cross(&v).magnitude() / line_length > tol {
                return false;
            }
        }
        true
    ...
    pub fn is_planar(&self, plane: Option<&mut Plane>, tolerance: Option<f64>) -> bool {
        ...
        for i in 0..self.m_cv_count {
            let p = self.get_cv(i).unwrap_or_default();
            let v = Vector::new(p[0] - p0[0], p[1] - p0[1], p[2] - p0[2]);
            if v.dot(&normal).abs() / normal.magnitude() > tol {
                return false;
            }
        }
    ...
    pub fn is_arc(&self, plane: Option<&mut Plane>, tolerance: Option<f64>) -> bool {
        ...
        if self.is_linear(Some(tol)) {
            return false;
        }
        let mut test_plane = Plane::default();
        if !self.is_planar(Some(&mut test_plane), Some(tol)) {
            return false;
        }
        ...
        for i in 0..=num_samples {
            ...
            if (self.point_at(t).distance(&center, None) - radius).abs() > tol {
                return false;
            }
        }
        if let Some(plane) = plane {
    ...
    pub fn is_in_plane(&self, test_plane: &Plane, tolerance: Option<f64>) -> bool {
        ...
        for i in 0..self.m_cv_count {
            ...
            if v.dot(&test_plane.z_axis()).abs() > tol {
                return false;
            }
        }
        true
    ...
    pub fn is_duplicate(
        ...
        for i in 0..self.m_cv_count {
            ...
            if p1.distance(&p2, None) > tol {
                return false;
            }
            if self.m_is_rat && (self.weight(i) - other.weight(i)).abs() > tol {
                return false;
            }
        }
        if !ignore_parameterization {
            for i in 0..self.nurbsknot_count() {
                if (self.m_nurbsknot[i] - other.m_nurbsknot[i]).abs() > tol {
                    return false;
                }
            }
        }
    ...
    /// The CV doubles, None when out of range
    ...
    pub fn trim(&mut self, t0: f64, t1: f64) -> bool {
        ...
        let stol = (d0.abs() + d1.abs() + (d1 - d0).abs()) * f64::EPSILON.sqrt();
