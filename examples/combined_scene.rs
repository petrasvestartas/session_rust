...
fn world_points(g: &Geometry, xf: &Xform) -> Vec<Point> {
    ...
    match g {
        ...
        Geometry::Mesh(m) => pts(xf, m.vertex.values().map(|v| Point::new(v.x, v.y, v.z)).collect()),
        Geometry::NurbsCurve(c) => {
            let (t0, t1) = c.domain();
            pts(xf, (0..=16).map(|i| c.point_at(t0 + (t1 - t0) * i as f64 / 16.0)).collect())
        }
        Geometry::NurbsSurface(s) => {
            ...
            for i in 0..=8 { for j in 0..=8 {
                ...
                if let Some(p) = s.point_at(u, v) { ps.push(p); }
            }}
            pts(xf, ps)
        }
        Geometry::BRep(b) => {
            let m = b.mesh();
            pts(xf, m.vertex.values().map(|v| Point::new(v.x, v.y, v.z)).collect())
        }
...
fn layer_of(src: &Session) -> HashMap<String, String> {
    ...
    let Some(root) = src.tree.root() else { return map };
...
fn place(dst: &mut Session, src: &Session, shift: &Xform, file: &str) -> usize {
    ...
    for guid in src.order() {
        let Some(g) = src.get_object(&guid) else { continue };
        ...
        match g {
            ...
            Geometry::Polyline(pl) => { dst.add_polyline(moved!(pl), parent); }
            Geometry::PointCloud(pc) => { dst.add_pointcloud(moved!(pc), parent); }
            Geometry::Mesh(m) => { dst.add_mesh(moved!(m), parent); }
            Geometry::NurbsCurve(c) => { dst.add_nurbscurve(moved!(c), parent); }
            Geometry::NurbsSurface(s) => { dst.add_nurbssurface(moved!(s), parent); }
            Geometry::BRep(b) => { dst.add_brep(moved!(b), parent); }
            Geometry::Plane(p) => { dst.add_plane(moved!(p), parent); }
            Geometry::OBB(o) => { dst.add_obb(moved!(o)); }
...
fn main() {
    ...
    for path in DRAWINGS {
        ...
        if s.lookup.is_empty() { println!("skip {path} - empty or missing"); continue; }
        files.push((s.name.clone(), s));
    }

    let boxes: Vec<Option<([f64; 3], [f64; 3])>> = files.iter().map(|(_, s)| bounds(s)).collect();
    let cell_x = boxes.iter().flatten().map(|(lo, hi)| hi[0] - lo[0]).fold(0.0, f64::max) + GAP;
    let cell_y = boxes.iter().flatten().map(|(lo, hi)| hi[1] - lo[1]).fold(0.0, f64::max) + GAP;
    ...
    let groups = out.tree.root().map(|r| r.borrow().children().len()).unwrap_or(0);
    println!("combined: {} objects, {} layer groups, cell {:.0} x {:.0} mm, {} cols",
        out.lookup.len(), groups, cell_x, cell_y, cols);
