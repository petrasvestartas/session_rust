...
fn main() {
    ...
    let before: HashMap<String, HashMap<usize, HashMap<usize, Option<usize>>>> = a
        .objects
        .meshes
        .iter()
        .map(|m| (m.guid().to_string(), m.halfedge.clone()))
        .collect();
    ...
    for m in &b.objects.meshes {
        ...
        assert_eq!(
            &m.halfedge,
            want,
            "halfedge rebuild differs for mesh {}",
            m.guid()
        );
        checked += 1;
    }
    ...
    for (x, y) in a.objects.lines.iter().zip(b.objects.lines.iter()) {
        ...
        for i in 0..6 {
            assert!((x[i] - y[i]).abs() < 1e-12, "line coords moved");
        }
    ...
    println!(
        "{:<32} {:>6.1} MB -> {:>6.1} MB   ({:.0}% smaller)  halfedge maps verified on {} meshes",
        path.rsplit('/').next().unwrap(),
        old.len() as f64 / 1e6,
        new.len() as f64 / 1e6,
        100.0 * (1.0 - new.len() as f64 / old.len() as f64),
        checked
    );
}
