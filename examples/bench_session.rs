...
use session_rust::{Session, Line, Geometry};
...
fn main() {
    ...
    println!("add_line x{}k   {:>7.0} ms   rss +{:.0} MB", N / 1000, t.elapsed().as_secs_f64() * 1e3, rss_mb() - m0);
    let t = Instant::now();
    let mut acc = 0.0;
    for g in s.lookup.values() { if let Geometry::Line(l) = g { acc += l.start()[0]; } }
    println!("iterate lookup  {:>7.1} ms   (acc {acc})", t.elapsed().as_secs_f64() * 1e3);
    let t = Instant::now();
    let bytes = s.pb_dumps();
    println!("pb_dumps        {:>7.0} ms   ({:.1} MB)", t.elapsed().as_secs_f64() * 1e3, bytes.len() as f64 / 1e6);
    let t = Instant::now();
    let s2 = Session::pb_loads(&bytes).unwrap();
    println!("pb_loads        {:>7.0} ms   ({} objects)", t.elapsed().as_secs_f64() * 1e3, s2.lookup.len());
    let t = Instant::now();
    let s3 = s.clone();
    println!("clone           {:>7.0} ms   ({} objects)", t.elapsed().as_secs_f64() * 1e3, s3.lookup.len());
